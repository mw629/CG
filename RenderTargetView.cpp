...
void RenderTargetView::CreateRenderTargetView(ID3D12Device *device, ID3D12Resource* swapChainResources, ID3D12DescriptorHeap* rtvDescriptorHeap)
{
	//RTVの設定
	...
	rtvHandles_[1].ptr = rtvHandles[0].ptr + device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
	//2つ目を作る
