...
PixelShaderOutput main(VertexShaderOutput input)
{
    ...
    for (int32_t x = 0; x < 3; ++x)
    {
        for (int32_t y = 0; y < 3; ++y)
        {
            ...
            float32_t3 color = gTexture.Sample(gSampler, texcoord).rgb;
            float32_t lum = Luminance(color);
    ...
    weight = saturate(weight * 10.0f); // Adjust weight for luminance difference
